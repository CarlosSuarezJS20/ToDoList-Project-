...
export class Task {
	...
	generateTaskHTML() {
		const taskTemplateElement = document.importNode(
			this.taskTemplateEl.content,
			true
		);
		...
		taskElement.addEventListener('dragstart', (event) => {
			...
			taskElement.parentElement.classList.add('droppable');
		});

		taskElement.addEventListener('dragleave', (event) => {
			event.dataTransfer.setData('text/plain', this.id);
			event.dataTransfer.effectAllowed = 'move';
			taskElement.parentElement.classList.remove('droppable');
		});
